...
{
  res_ = response::stock_response(response::ok);
...
{
  res_ = response::stock_response(response::created);
  EXPECT_EQ(res_.code, response::created);
...
{
  res_ = response::stock_response(response::accepted);
  EXPECT_EQ(res_.code, response::accepted);
...
{
  res_ = response::stock_response(response::no_content);
  EXPECT_EQ(res_.code, response::no_content);
...
{
  res_ = response::stock_response(response::multiple_choices);
  EXPECT_EQ(res_.code, response::multiple_choices);
...
{
  res_ = response::stock_response(response::moved_permanently);
  EXPECT_EQ(res_.code, response::moved_permanently);
...
{
  res_ = response::stock_response(response::moved_temporarily);
  EXPECT_EQ(res_.code, response::moved_temporarily);
...
{
  res_ = response::stock_response(response::not_modified);
  EXPECT_EQ(res_.code, response::not_modified);
...
{
  res_ = response::stock_response(response::bad_request);
...
{
  res_ = response::stock_response(response::unauthorized);
  EXPECT_EQ(res_.code, response::unauthorized);
...
{
  res_ = response::stock_response(response::forbidden);
  EXPECT_EQ(res_.code, response::forbidden);
...
{
  res_ = response::stock_response(response::not_found);
...
{
  res_ = response::stock_response(response::internal_server_error);
  EXPECT_EQ(res_.code, response::internal_server_error);
...
{
  res_ = response::stock_response(response::not_implemented);
  EXPECT_EQ(res_.code, response::not_implemented);
...
{
  res_ = response::stock_response(response::bad_gateway);
  EXPECT_EQ(res_.code, response::bad_gateway);
...
TEST_F(ResponseTest, HandlesServiceUnavailableResponse)
 {
  res_ = response::stock_response(response::service_unavailable);
  EXPECT_EQ(res_.code, response::service_unavailable);
...
{
  res_.code = response::multiple_choices;
  std::vector<boost::asio::const_buffer> buffers = res_.to_buffers();
...
{
  res_.code = response::moved_permanently;
  std::vector<boost::asio::const_buffer> buffers = res_.to_buffers();
...
{
  res_.code = response::moved_temporarily;
  std::vector<boost::asio::const_buffer> buffers = res_.to_buffers();
...
{
  res_.code = response::not_modified;
  std::vector<boost::asio::const_buffer> buffers = res_.to_buffers();
...
TEST_F(ResponseTest, HandlesConvertingBadRequestReplyToBuffer)
 {
  res_.code = response::bad_request;
  std::vector<boost::asio::const_buffer> buffers = res_.to_buffers();
...
{
  res_.code = response::unauthorized;
  std::vector<boost::asio::const_buffer> buffers = res_.to_buffers();
...
{
  res_.code = response::internal_server_error;
  std::vector<boost::asio::const_buffer> buffers = res_.to_buffers();
...
{
  res_.code = response::not_implemented;
  std::vector<boost::asio::const_buffer> buffers = res_.to_buffers();
...
{
  res_.code = response::bad_gateway;
  std::vector<boost::asio::const_buffer> buffers = res_.to_buffers();
...
{
  res_.code = response::service_unavailable;
  std::vector<boost::asio::const_buffer> buffers = res_.to_buffers();
