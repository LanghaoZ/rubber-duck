...
//   try 
//   {
//     std::make_shared<http::server::session>(std::move(socket_), manager_, handlers_)->start();
//   }
...
//   // instantiate session
//   auto session = std::make_shared<http::server::session>(std::move(socket_), manager_, handlers_);
